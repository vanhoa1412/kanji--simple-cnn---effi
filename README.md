# 🖌️ Kanji - Nhận diện chữ Kanji viết tay
...
Trước khi chạy chương trình, hãy đảm bảo máy tính đã cài đặt Python (phiên bản 3.8 trở lên).
...
Khởi động chương trình bằng lệnh:
...
Vẽ chữ: Dùng chuột (nhấn giữ chuột trái) vẽ chữ Kanji vào khung màu đen.
Lưu ý: Nên vẽ nét dứt khoát, to và rõ ràng.
Nhận diện: Thả chuột ra, kết quả dự đoán sẽ hiện ngay bên dưới kèm theo độ tin cậy (%).
Xóa bảng: Nhấn nút Xóa bảng (hoặc Clear) để vẽ chữ mới.
Xử lý khi máy đoán sai:
Thử tích vào ô [x] Đảo màu (nếu model hiện tại được train với nền trắng chữ đen).
Kiểm tra xem hình vẽ có bị mất nét hoặc quá nhỏ không.
3. Chuyển đổi Model
Để đổi giữa EfficientNet và Simple CNN, bạn mở file test.py bằng trình soạn thảo code, tìm phần CẤU HÌNH ở đầu file và bỏ comment (dấu #) dòng model bạn muốn dùng:
```bash
# Chọn 1 trong 2 dòng dưới đây:
MODEL_PATH = "best_efficientnet_kanji.h5"  # Dùng EfficientNet (Khuyên dùng - Chính xác cao)
# MODEL_PATH = "best_simple_cnn.h5"        # Dùng Simple CNN (Nhẹ - Nhanh)
```
